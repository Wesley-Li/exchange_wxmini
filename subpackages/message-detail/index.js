...
Page({
  ...
  async getPassword() {
    ...
    if(res.retcode==0){
        that.setData({
          ...
          var tim = app.globalData.tim
          let promise = tim.login({userID: res.tid, userSig: res.data})
          promise.then(res => {
            console.log('登录成功')
            wx.setStorageSync('isImLogin', true)
            app.globalData.isImLogin = true
  ...
  //创建自定义商品消息体
  async createProductmsg(){
    var that = this;
    var id = that.data.houseId
    const goodsDetailRes = await WXAPI.goodsDetail(id)
    
    if (goodsDetailRes.retcode == 0) {
      console.log(goodsDetailRes.data.video)
      const params =  {
        house_pic: goodsDetailRes.data.pic,
        area: 0,
        price: goodsDetailRes.data.credprice,
        park: '',
        city: '',
      ...
      const option = {
        ...
        payload: {
          data: JSON.stringify(params),// 自定义消息的数据字段
          description: params.title, // 自定义消息的说明字段
          extension: params.price.toString() // 自定义消息的扩展字段
        } // 消息内容的容器
      }
      const tim = app.globalData.tim
      // 2. 创建消息实例，接口返回的实例可以上屏
